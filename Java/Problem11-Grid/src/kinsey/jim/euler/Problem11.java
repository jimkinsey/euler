...
public class Problem11 {
	...
	private static final String problemGrid = 
	...
	public static void main(String[] args) {
		...
		Grid<Integer> grid = new Grid<Integer>(20, 20);
		populateGridFromString(grid, problemGrid);
		
		for (int y = 0; y < grid.height; y++) {
			for (int x = 0; x < grid.width; x++) {
				for (List<Integer> line : getLines(grid, x, y, 4)) {
					max = (int) Math.max(max, product(line));
				}
			}
		}
	...
	private static List<List<Integer>> getLines(Grid<Integer> grid, int x, int y, int length) {
		...
		for (Direction direction : validDirections(grid,x,y,length)) {
			lines.add(grid.getLine(x, y, length, direction));
		}
		
		return lines;
	}
	
	private static Set<Direction> validDirections(Grid<Integer> grid, int x, int y, int length) {
		Set<Direction> directions = new HashSet<Direction>();
		
		if (x <= grid.width - length) {
			directions.add(Direction.E);
			
			if (y <= grid.height - length) {
	...
	private static List<Integer> getRowFromString(int row, int width, String gridDescription) {
		List<String> numberStrings = Arrays.asList(gridDescription.split(" "));
		List<String> numberStringsInRow = numberStrings.subList(row * width, (row * width) + width);
		
		return numbersFromStrings(numberStringsInRow);
	}
	
	private static List<Integer> numbersFromStrings(List<String> strings) {
